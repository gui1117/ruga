...
impl From<toml::de::Error> for LoadCastlesError {
    fn from(err: toml::de::Error) -> LoadCastlesError {
        LoadCastlesError::Toml(err)
    }
}
impl From<::std::io::Error> for LoadCastlesError {
    fn from(err: ::std::io::Error) -> LoadCastlesError {
        LoadCastlesError::Io(err)
    }
}
...
pub fn load_castles(mut musics: Vec<String>) -> Result<(Vec<Castle>, Vec<String>), LoadCastlesError> {
    ...
    for dir_entry in fs::read_dir(config.levels.dir.clone())? {
        ...
        let mut file = fs::File::open(dir_entry.path().join(String::from("config.toml")))?;
